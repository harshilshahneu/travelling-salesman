...
public class MinimumWeightPerfectMatching {
	...
	/**
	 * Get the minimum weight cost matching. The matching is returned as an array
	 * where the nodes at index 2*i and 2*i+1 form a matched pair. For example,
	 * nodes at indexes (0, 1) are a pair, (2, 3) are another pair, etc...
	 *
	 * <p>
	...
	public void solve() {
		...
		// i'th node is
		// included in the state. Encoding the state this way allows us to compactly
		// represent selecting
		// a subset of the nodes present in the matching. Furthermore, it allows using
		// the '&' binary
		// operator to compare states to see if they overlap and the '|' operator to
		// combine states.
		//
		// dp[i] contains the optimal cost of the MWPM for the nodes captured in the
		// binary
		// representation of `i`. The dp table is always half empty because all states
		// with an odd
		// number of nodes do not have a MWPM.
		Double[] dp = new Double[1 << n];

		// Memo table to save the history of the chosen states. This table is used to
		// reconstruct the
		// chosen pairs of nodes after the algorithm has executed.
		int[] history = new int[1 << n];

		// All the states consisting of pairs of nodes are the building blocks of this
		// algorithm.
		// In every iteration, we try to add a pair of nodes to previous state to
		// construct a larger
		// matching.
		...
		for (int state = 0b11; state < (1 << n); state++) { // O(2^n)
			...
			for (int i = 0; i < numPairs; i++) { // O(n^2)
				...
				if (dp[newState] == null || newCost < dp[newState]) {
					dp[newState] = newCost;
					// Save the fact that we went from 'state' -> 'newState'. From this we will be
					// able to
					// reconstruct which pairs of nodes were taken by looking at 'state' xor
					// 'newState' which
	...
	// Populates the `matching` array with a sorted deterministic matching sorted by
	// lowest node
	// index. For example, if the perfect matching consists of the pairs (3, 4), (1,
	// 5), (0, 2).
	// The matching is sorted such that the pairs appear in the ordering: (0, 2),
	// (1, 5), (3, 4).
	...
	private void reconstructMatching(int[] history) {
		...
		// Reconstruct the matching of pairs of nodes working backwards through computed
		// states.
	...
	// Gets the zero base index position of the 1 bit in `k`. `k` must be a power of
	// 2, so there is
	...
	private static void test1() {
		...
		for (int i = 0; i < matching.length / 2; i++) {
			...
			System.out.printf("(%d, %d) <-> (%d, %d)\n", (int) pts.get(ii).getX(), (int) pts.get(ii).getY(),
					(int) pts.get(jj).getX(), (int) pts.get(jj).getY());
		}
	}
