...
public class TSPDynamicProgramming {
	private static final Logger LOGGER = LoggerFactory.getLogger(TSPDynamicProgramming.class);
	...
	public TSPDynamicProgramming(int start, List<Point> points) {
		N = points.size();
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < N; j++) {
				...
				double cost = HaversineDistance.haversine(source, destination);
				distance[i][j] = cost;
			}
		}
	...
	public static void main(String[] args) {
		...
		int startNode = 0;
		TSPDynamicProgramming solver = new TSPDynamicProgramming(startNode, distanceMatrix);

		// Prints: [0, 3, 2, 4, 1, 5, 0]
		LOGGER.trace("Tour: {}", solver.getTour());

		// Print: 42.0
		LOGGER.trace("Tour cost: {}", solver.getTourCost());
	}
